...
const CATEGORY_INFO = {
  ...
  featuredSeries: {
    label: 'Be Sure to Follow The Featured Motorsports Series!',
    icon: '🏎️',
    info: `Check out the latest and greatest motorsports series featured by Vibe Racing!\n\nFollow these series:\n\n- SRO Motorsports: https://www.sro-motorsports.com/\n- Formula 1: https://www.formula1.com/\n- FIA WEC: https://www.fiawec.com/\n- Formula Drift: https://www.formulad.com/`
  },
};
...
function renderCategories() {
  ...
  Object.entries(CATEGORY_INFO).forEach(([key, cat]) => {
    if (state.page === 'products' && (key === 'contact' || key === 'social' || key === 'address' || key === 'featuredSeries')) {
      return; // Skip these buttons in Products section
    }
    ...
    if (key === 'featuredSeries') {
      btn.onclick = () => {
        // Show clickable links modal
        let modal = document.getElementById('featured-modal');
        ...
        modal.style.boxShadow = '0 4px 24px #000';
        modal.innerHTML = `
          <h2 style="color:#39ff14; margin-bottom:1rem;">${cat.label}</h2>
          <p>Check out the latest and greatest motorsports series featured by Vibe Racing!</p>
          <ul style="list-style:none; padding:0;">
            <li><a href="https://www.sro-motorsports.com/" target="_blank" style="color:#39ff14; text-decoration:underline;">SRO Motorsports</a></li>
            <li><a href="https://www.formula1.com/" target="_blank" style="color:#39ff14; text-decoration:underline;">Formula 1 🏎️</a></li>
            <li><a href="https://www.fiawec.com/" target="_blank" style="color:#39ff14; text-decoration:underline;">FIA WEC</a></li>
            <li><a href="https://www.formulad.com/" target="_blank" style="color:#39ff14; text-decoration:underline;">Formula Drift 🛞💨</a></li>
          </ul>
          <button id="close-featured-modal" style="margin-top:1.5rem; background:#39ff14; color:#222; border:none; border-radius:6px; padding:0.5rem 1.2rem; cursor:pointer; font-size:1rem;">Close</button>
        `;
        document.body.appendChild(modal);
        document.getElementById('close-featured-modal').onclick = function() {
          modal.remove();
        };
      };
    } else {
...
function renderDropdownMenu() {
  ...
  menu.style.minWidth = '180px';
    menu.innerHTML = `
      ...
      <div class="menu-item" style="cursor:pointer; color:#39ff14;" onclick="window.showCategoryInfo('featuredSeries')">🏎️ Be Sure to Follow The Featured Motorsports Series!<br><span style='font-size:1rem;'>Formula 1 🏎️</span></div>
    `;
  window.showCategoryInfo = function(key) {
    const cat = CATEGORY_INFO[key];
    if (!cat) return;
    if (key === 'featuredSeries') {
      // Create a modal for clickable links
      let modal = document.getElementById('featured-modal');
      if (modal) modal.remove();
      modal = document.createElement('div');
      modal.id = 'featured-modal';
